// NOTE: taken from tree-sitter-rust
...
void *tree_sitter_cbnf_external_scanner_create() {
  return ts_calloc(1, sizeof(Scanner));
}

void tree_sitter_cbnf_external_scanner_destroy(void *payload) {
  ts_free((Scanner *)payload);
}

unsigned tree_sitter_cbnf_external_scanner_serialize(void *payload,
                                                     char *buffer) {
...
void tree_sitter_cbnf_external_scanner_deserialize(void *payload,
                                                   const char *buffer,
                                                   unsigned length) {
  Scanner *scanner = (Scanner *)payload;
...
static inline void
process_left_forward_slash(BlockCommentProcessing *processing, char current) {
  if (current == '*') {
...
static inline void process_continuing(BlockCommentProcessing *processing,
                                      char current) {
  switch (current) {
  case '/':
...
static inline bool process_block_comment(TSLexer *lexer,
  ...
  if (valid_symbols[BLOCK_OUTER_DOC_MARKER] && first == '*') {
    ...
    // BLOCK_OUTER_DOC_MARKER as BLOCK_OUTER_DOC_MARKER's only have 2 * not 3 or
    // more.
    if (lexer->lookahead != '*') {
  ...
  if (valid_symbols[BLOCK_COMMENT_CONTENT]) {
    ...
    // For the purposes of actually parsing cbnf code, this
...
bool tree_sitter_cbnf_external_scanner_scan(void *payload, TSLexer *lexer,
                                            const bool *valid_symbols) {
